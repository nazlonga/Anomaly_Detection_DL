...
def kl_loss(inputs,outputs):
    #z_mean, z_log_var = args
    kl_loss = - 0.5 * K.mean(1 + z_log_var - K.square(z_mean) - K.exp(z_log_var), axis=-1) ## replacing K.mean for K.sum
    return kl_loss
...
vae.compile(loss=total_loss, optimizer='rmsprop', metrics=[mean_squared_error, kl_loss, binary_crossentropy])
vae.summary()
...
pickle.dump({'train': h.history}, open('VAE_3D_BCE_100epoch_KL_mean_noimg_history.pkl', 'wb'))


vae.save_weights('VAE_3D_BCE_100epoch_KL_mean_noimg_weights.hdf5')
model_json = vae.to_json()
with open("VAE_3D_BCE_100epoch_KL_mean_noimg_model.json", "w") as json_file:
    json_file.write(model_json)
